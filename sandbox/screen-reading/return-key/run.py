#!/usr/bin/env python3
"""return-key experiment: launches MAME and collects screen buffer log."""
...
project_root = sandbox_dir.parent.parent
...
env = os.environ.copy()
env["SANDBOX_DIR"] = str(sandbox_dir)
env["LOG_FILE"] = str(log_file)

print("Launching MAME...", file=sys.stderr)
print("Click the game window to enter live mode when the demo starts.", file=sys.stderr)
...
if log_file.exists():
    ...
    if len(lines) > 6:
        print(f"  ... ({len(lines) - 3} more lines)", file=sys.stderr)
else:
